...
module.exports = {
    ...
    run: async (interaction) => {
        ...
        MysqlMerpati.query(`SELECT * FROM ucp WHERE DiscordID = '${userid}'`, async (error, row) => {
            if (row[0]) {
                ...
                MysqlMerpati.query(`UPDATE ucp SET verifycode = '${randCode}' WHERE DiscordID = ${userid}`);

                const msgEmbed = new MessageEmbed()
                    .setAuthor({ name: "Metro Roleplay", iconURL: client.config.ICON_URL })
                    ...
                    .setColor("GOLD")
                    .setFooter({ text: "Bot Kota METRO ROLEPLAY" })
                    .setTimestamp()
